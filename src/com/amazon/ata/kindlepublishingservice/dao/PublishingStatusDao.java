...
public class PublishingStatusDao {
    ...
    public List<PublishingStatusItem> getPublishingStatus(String publishingRecordId) {
        ...
        PaginatedQueryList<PublishingStatusItem> statusItems = dynamoDbMapper.query(PublishingStatusItem.class,
                                                                                    queryExpression);
        if (statusItems.size() == 0) {
            throw new PublishingStatusNotFoundException("item " + publishingRecordId + "was not found");
        }
        return statusItems;
    }
}
